//
//  VLJuliaControlFunctionCBSignalingModelStrategy.m
...
#import "VLJuliaControlFunctionCBSignalingModelStrategy.h"

#define NEW_LINE [buffer appendString:@"\n"]

@implementation VLJuliaControlFunctionCBSignalingModelStrategy
...
-(id)executeStrategyWithOptions:(NSDictionary *)options
{
    ...
    // Populate the buffer --
    [buffer appendFormat:@"function %@(time_value,XCURRENT,FLOW,DFIN)\n",tmpFunctionName];
    [buffer appendString:@"# ---------------------------------------------------------------------- #\n"];
    ...
    for (NSXMLElement *species_node in species_array)
    {
        ...
        [buffer appendFormat:@"%@ = XCURRENT[%lu];\n",species_id,counter];
    ...
    {
        [buffer appendFormat:@"LB[%lu] = ALPHA[%lu]",reaction_counter,reaction_counter];
    ...
    for (NSXMLElement *reaction_node in reaction_array)
    {
        [buffer appendFormat:@"UB[%lu] = BETA[%lu]",reaction_counter,reaction_counter];
    ...
    NEW_LINE;
    [buffer appendString:@"return LB,UB;\n"];
